  ...
  <div class="relative min-h-screen bg-slate-100">
    ...
    <header class="sticky top-0 z-40 border-b border-slate-200/70 bg-white/85 backdrop-blur-xl">
      <div class="mx-auto w-full max-w-[1480px] px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <div class="hidden md:flex items-center gap-2 rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-600">
          Deposito Central -> Assu / Mossoro
        </div>
      </div>
    </header>

    <main class="relative mx-auto w-full max-w-[1480px] px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
      <slot />
    </main>
  </div>
</template>
